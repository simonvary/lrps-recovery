function [L S err errTrue iter_timings] = sparcs(b, r, K, A, At, svdMethod, tol, maxIter, VERBOSE,tol_early, X_true)
%%%%
%Solves the problem
...
iter_timings = zeros(maxIter,1);
errTrue = zeros(maxIter,1);
tic;
...
while ((err>tol)&&(iterCnt<=maxIter))
    ...
    iter_timings(iterCnt) = toc;
    errTemp = norm(A(Xhat)+A(s_cosamp)-b)/norm(b);
    
    errTrue(iterCnt) = relerr(Xhat(:) + s_cosamp(:), X_true(:));
...
iter_timings = iter_timings(1:iterCnt);
errTrue = errTrue(1:iterCnt);
